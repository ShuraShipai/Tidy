...
The scan snapshot is one shared source for Home, Photos, Videos and Contacts. The first scan and Scan Again are explicit full scans. Restore the last successful lightweight snapshot at launch, then reconcile Photos and Contacts asynchronously: compare asset identifiers/metadata, analyze only changed or new media and affected similarity windows, and update only the changed domain. A verified in-app media deletion removes exactly the confirmed IDs and their pair edges from the shared snapshot and persists that result; it never starts a full scan. Library notifications and foreground checks reconcile changes without clearing unrelated findings. Revoked access hides that domain immediately; restored access must be revalidated before its findings are actionable. A failed or interrupted full scan must leave the prior successful snapshot available. iOS may suspend or terminate a running check, so resume from the saved snapshot on next launch rather than assuming background execution.
